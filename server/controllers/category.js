...
exports.update = async (req, res) => {
  ...
  try { 
    ...
    if (image) { 
      ...
      updatedCategory.image.url = `https://myextendedbucket.s3.${process.env.AWS_S3_REGION}.amazonaws.com/${newImageKey}`;
      updatedCategory.image.key = newImageKey;
    }
 
    const savedCategory = await updatedCategory.save();
    console.log("UPDATED CATEGORY WITH NEW IMAGE:", savedCategory);
...
exports.remove = async (req, res) => {
  ...
  try {
    ...
    if (categoryToDelete.image && categoryToDelete.image.key) {
      const deleteParams = {
        Bucket: "myextendedbucket",
        Key: `${categoryToDelete.image.key}`,
      };

      const deleteCommand = new DeleteObjectCommand(deleteParams);
      const deleteData = await s3.send(deleteCommand);
      console.log("categoryToDelete.image.key", categoryToDelete.image.key);
